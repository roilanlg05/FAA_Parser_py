...
from app.tbfm_db import TbfmAsyncSessionLocal
from app.tbfm_models import TbfmEvent, TbfmProjection
from app.tbfm_payload_utils import strip_raw_fields
    ...
    while True:
        ...
        async with TbfmAsyncSessionLocal() as session:
            result = await session.execute(
                select(TbfmEvent).where(TbfmEvent.id > last_id).order_by(TbfmEvent.id.asc()).limit(batch_size)
            )
...
async def backfill_projections(batch_size: int, max_batches: int | None) -> tuple[int, int]:
    ...
    while True:
        ...
        async with TbfmAsyncSessionLocal() as session:
            result = await session.execute(
                select(TbfmProjection)
                .where(TbfmProjection.projection_key > last_key)
                .order_by(TbfmProjection.projection_key.asc())
                .limit(batch_size)
            )
            rows = list(result.scalars())
            if not rows:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Backfill TBFM compact JSON payloads (strip embedded raw branches)')
